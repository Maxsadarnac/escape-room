...
/* ---- Clue templates -------------------------------------------------------
   Each clue is { text, test } where test(perm) checks a candidate
   arrangement (perm[position] = item index). Every clue is generated FROM
   the secret, so the secret always satisfies the full pool.               */
...
function buildCluePool(secret, names) {
  ...
  for (let pos = 0; pos < n - 1; pos++) {
    const a = secret[pos];
    const b = secret[pos + 1];
    clues.push({
      kind: "adjacent",
      text: `${cap(names[a])} stands immediately to the left of ${names[b]}.`,
      test: (perm) => perm.indexOf(a) + 1 === perm.indexOf(b),
    });
  }

  // Ends — strong anchors, at most one of each offered.
  clues.push({
    kind: "end",
  ...
  clues.push({
    kind: "end",
    text: `${cap(names[secret[n - 1]])} stands at the far right.`,
    test: (perm) => perm[n - 1] === secret[n - 1],
  });

  // Not-at-either-end, for every interior item.
  for (let pos = 1; pos < n - 1; pos++) {
    const item = secret[pos];
    clues.push({
      kind: "notEnd",
      text: `${cap(names[item])} stands at neither end.`,
      test: (perm) => perm[0] !== item && perm[n - 1] !== item,
    });
  }
  ...
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < n; j++) {
      if (posOf[i] + 1 < posOf[j]) {
        clues.push({
          kind: "before",
          text: `${cap(names[i])} stands somewhere to the left of ${names[j]}.`,
          test: (perm) => perm.indexOf(i) < perm.indexOf(j),
        });
      }
    }
  }
  ...
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      if (Math.abs(posOf[i] - posOf[j]) >= 2) {
        clues.push({
          kind: "notAdjacent",
          text: `${cap(names[i])} and ${names[j]} do not stand beside each other.`,
          test: (perm) => Math.abs(perm.indexOf(i) - perm.indexOf(j)) !== 1,
        });
      }
    }
  }
